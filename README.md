...
**外观与交互（v1.22 苹果风改版）**
- **Acrylic 真毛玻璃**：Win11 下经 `SetWindowCompositionAttribute` 开启系统级背景模糊 + 暗色 tint，壁纸/窗口实时透出；`SetWindowRgn` 裁剪 12px 连续圆角；老系统回退 PIL 自绘卡片
...
- **hover 详情面板**：悬停 0.4 秒从卡片左缘滑出第二块毛玻璃面板，显示 GLM/Kimi 精确百分比、窗口重置时刻、番茄钟阶段与上次刷新时间

**用量监控（上段三列）**
- 第一列：竖排「TOKEN」文字标签（纯标识）+ 竖发丝线与数据列分组
- **GLM 列**（顶部「GLM + 短期剩余小数字」）三条竖向进度条，底部锚定向上填充：
  1. 短期 Token 剩余（`TOKENS_LIMIT unit=3` 的 5h 窗口，iOS 系统色分级：≥40% 绿 / 15-40% 橙 / <15% 红，数字同步染红）
  2. 5h 窗口重置倒计时（黄填充 = 距重置剩余时间；满=刚重置回满，空=即将重置；整分心跳脉冲，见下）
  3. 周额度剩余（`unit=6`，红色）
- **KIMI 列**（顶部「KIMI + 短期剩余小数字」）三条竖向进度条：短期剩余 + 5h 重置倒计时（黄色，同样整分心跳）+ 月额度剩余（红色，`monthTotal`，与 GLM 周条语义平行的长周期额度）。数据来自 Kimi Code CLI 的本地 daemon（`~/.kimi-code` 下动态端口 + `server.token` 鉴权，读 `limit5h` / `monthTotal` 的 `usedRatio` / `resetAt`）；daemon 未运行时显示空轨道 + 「--」，不伪造读数
- 黄条**整分心跳**：5h 摊在竖条满刻度上位移肉眼不可见，改为每分钟整分向白色脉冲一下再 6 秒渐回，以颜色节律传达倒计时在走
- 满 5h 满刻度起步、随每次拉取自动校准；tooltip 显示「短期剩余%（XhXXm 后重置）| 周额度剩余% | Kimi 短期剩余%（XhXXm 后重置）」
