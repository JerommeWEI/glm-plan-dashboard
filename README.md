...
- **左右贴边自动隐藏**：卡片左/右缘进入所在屏对应竖边 24px 内（两边都够到取更近者）松手，空闲 1.2 秒滑入该侧屏缘，露出 6px **竖向番茄进度条**（水位式：满格齐顶、随剩余时间自上而下回落，落空即到点；剩余 >50% 绿 / 20–50% 黄 / <20% 红）；鼠标触及该侧屏缘即滑出（启动后 5 秒宽限期不隐藏；拖离屏缘后不启用）。左/右停靠全对称：细边热区、region 裁剪、所在屏判定锚点、hover 面板展开方向（左停靠向右展开）随停靠侧镜像，跨屏拖动到哪块屏就贴哪块屏，停靠侧与位置记忆在 `ui_state.json`（旧文件无 `side` 字段按右停靠兼容）
- **hover 详情面板**：悬停单个图标 0.3 秒，在其靠屏内一侧展开 168×64 的局部详情卡片；切换图标只更新对应项，离开轨道和卡片 0.3 秒后收起

**状态轨（v1.28）**
- 44px 宽的完整胶囊内固定四枚圆环：GLM 5h 短期额度（深蓝晶体）、Kimi 5h 短期额度（绿色星芒）、Codex 周预算（洋红终端符）、当前番茄阶段剩余（工作紫 / 休息 teal / 非工作灰）。每枚图标下方显示对应剩余百分比，图标和位置保持固定，便于形成空间记忆。
- 平台主色不随额度变化，低于 40% 时用橙色小点提示，低于 15% 时用红色小点提示；精确百分比、重置时间、GLM 周额度、Kimi 月额度和 Codex 周绝对值（$已用/$上限）与今日消耗全部保留在 hover 详情中。
- Kimi 本地 daemon、Codex 中转不可用时显示灰色虚线占位环，真实额度耗尽仍是红色警示点；番茄非工作时段使用灰色沙漏环，不把它误报为额度耗尽。
- 主轨只在状态变化时重绘：番茄钟每秒一帧、额度刷新后的缓动才 30fps，保留原有离屏 Pillow + Win32 分层窗口路径，无额外运行时依赖。
- 每 5 分钟自动刷新数据，三源（GLM 走配置的 API、Kimi 走本地 daemon、Codex 走中转站 `/v1/usage`）各自独立线程拉取与落地，任一源超时不拖慢其余源的 UI 更新

**Codex 用量源（v1.28）**
- 数据来自 Codex CLI 所配中转站（`~/.codex/config.toml` 顶层 `model_provider` 指向供应商的 `base_url`，白名单 `uapi.ccwu.cc`；key 为 `~/.codex/auth.json` 的 `OPENAI_API_KEY`，Bearer 鉴权），请求 `GET {base}/v1/usage`
- 额度为 USD 计价订阅，无 5h 窗口：环语义取**周预算剩余比例**（如 (300-17.4)/300），hover 详情给「周 $17.4/$300 · 6天9h 后预计重置 · 今日 $17.4」；`daily_limit_usd=0` 且 `mode=unrestricted` 表示无日限额，今日已用只以文字呈现、绝不折算成比例
- 周重置时刻由 `weekly_window_start + 7 天` 推算（服务端无显式 reset 字段），窗口起点在未来或已越过推算点时显示「等待窗口」而非编造倒计时；安全策略沿用 GLM 源（https + 域名白名单 + DNS 后私网阻断 + 禁重定向 + 响应体 64KB 上限）
...
**通用**
- 右键菜单：立即刷新 / 退出
- 可拖拽移动位置（拽着隐藏态细边拖动，松手处就地恢复整卡；松手时就近更新停靠侧）
- 圆角半透明悬浮窗口
- 支持开机自启动（登录后启动）
...
- **细边进度条改水位式**：竖向番茄进度条由顶部锚定（时间越少从底部上缩）改为水位自上而下回落——满格齐顶、随剩余时间下降，落空即到点，方向与「倒计时流逝」直觉一致；口径不变（当前番茄阶段 45/15 分钟归一，非工作时段空轨道）
- 拽着隐藏态细边拖动后松手：就地恢复整卡视觉（此前 6px 细条会悬在屏中间既唤不出也不贴边）；拖拽期间不再触发滑出动画，落点统一由松手决定；松手时就近更新停靠侧

### v1.28
- **新增 Codex（中转 GPT 订阅）第 4 槽**：洋红终端符 ❯_ 图标圆环显示周预算剩余（USD 计价无 5h 窗口，环语义仍为剩余递减）；hover 详情给「周 $已用/$上限 · N天Mh 后预计重置 · 今日 $消耗」，无日限额只以文字标注不折算比例。方案经 Codex/DS/Kimi 三方圆桌评审定稿（纪要见 `docs/codex-slot-roundtable.md`）
- **布局紧凑化**：44×232 → 44×252 四槽（槽距 75→60）；槽位常量合并为单一 `SLOT_SPECS` 派生（此前加槽需同步 5 处）
- **刷新链改每源独立线程**：GLM/Kimi/Codex 各自拉取、各自 `root.after` 落地，任一源一次 10s 超时不再连带推迟其余源 UI 更新（旧实现顺序 fetch）
- **数据健壮性**：tomllib 读 config.toml（供应商名跟顶层 `model_provider` 走不写死）、auth.json key 容错（Bearer 前缀剥离）；SSRF 校验沿用 GLM 策略并加响应体 64KB 上限；周重置推算带合理性校验（窗口起点在未来/已过期均不推算）
