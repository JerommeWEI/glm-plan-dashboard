# GLM Plan Dashboard v1.20

GLM 套餐用量悬浮小组件 + 番茄工作闹钟，Apple 风格竖版卡片贴靠屏幕右缘竖直居中：上段显示短期 Token 剩余量，右缘显示周额度剩余量，下段显示番茄钟倒计时。

## 功能

**Token 用量（上段）**
- 「TOKEN」14px 粗体标签 + 大号剩余百分比数字 + 5px 胶囊进度条
- 主数字固定读取 5 小时短期窗口（`unit=3`）的剩余量；不再被周窗口覆盖
...
- 黄条**整分心跳**：5h 摊在 56px 条宽上每分钟仅收缩 0.2px、位移肉眼不可见，改为每分钟整分向白色脉冲一下再 6 秒渐回（`_quota_pulse`），以颜色节律传达倒计时在走；无重置数据（空轨道）与番茄钟闪烁期间不跳
- 仅当服务端为短期窗口返回 `nextResetTime` 时才显示黄色倒计时；未返回时保持空轨道，表示重置时间未知，而非即将重置
- 卡片右缘新增红色竖向细条：读取周窗口（`unit=6`）的**剩余**比例，底部锚定；额度下降时上沿从上向下收缩，仅作次要提示
- 每 5 分钟自动刷新数据
